...
export async function updateGlucose(realm) {
    ...
    const fromDate = latestGlucose === null ? getLastMonthDate() : new Date(latestGlucose);
        ...
        await realm.write(() => {
            for (const glucoseInfo of result) {
                if (glucoseInfo.sgv != null && glucoseInfo.dateString != null) {
                    realm.create("GlucoseInfo", { glucose: glucoseInfo.sgv, timestamp: glucoseInfo.dateString });
                }
            }
        });
        ...
        await realm.write(() => {
            for (const insulinInfo of result) {
                if (insulinInfo.insulin != null && insulinInfo.created_at != null) {
                    realm.create("InsulinInfo", { insulin: insulinInfo.insulin, timestamp: insulinInfo.created_at });
                }
            }
        });
...
export async function createOrUpdateFood(realm, name, calories, carbohydrates, protein, fat) {
