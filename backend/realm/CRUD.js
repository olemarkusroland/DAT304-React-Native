  ...
  if (basal.length > 0) {
    for (let i = 0; i < basal.length; i++) {
      ...
      if (current_newest._j == date_time) {
        ...
        if (basal[i].store['Pattern 1'].basal.length != 0) {
          dateStrings.push(basal[i].created_at);
        }
        ...
        for (let j = 0; j < basal[i].store['Pattern 1'].basal.length; j++) {
          ...
          dateAddstrings.push(
            new Date(dateObject.getTime() + parseInt(timeStrings[j]) * 1000),
          );
...
export async function deleteExerciseByTimestamp(realm, timestamp) {
  const entriesToDelete = realm
    .objects('ExercicesInfo')
    .filtered('start > $0', timestamp);
